...
// NOTE: the gesture only fires on a theme that opts in via states.dizzy +
// timings.autoReturn.dizzy AND has eye tracking enabled (see THEME_SUPPORTS_DIZZY
// in refreshTheme). No built-in theme currently ships a dizzy asset, so the
// detector stays dormant until a theme provides one — the logic is complete and
// tested regardless.
...
function refreshTheme() {
  ...
  // its math on themes that don't define a real dizzy state (e.g. Cloudling).
...
function runMainTickOnce() {
    ...
    // themes skip the math entirely and keep normal idle behavior.
    //
